...
flags.DEFINE_string('weights_bats', './checkpoints/yolov4-416',
                    'path to bats weights file')
...
flags.DEFINE_string('directory', './data/video', 'path to input directory')
...
def main(_argv):
    ...
    env_input_size = 416 #input size for enviorment detection
    
    ### running on directory of videos
    ...
    for filename in os.listdir(directory):
        ...
        video_path = os.path.join(directory, filename).replace("\\","/")
        print("video_path: " , video_path)
        # checking if it's a file with an mp4 extension
        if os.path.isfile(video_path) and extension == ".mp4":
            print("yep")
            ...
            ### output files init ###   
            output = directory + "/" + "output_" + video_name + ".avi"
            logger_utils.delete_if_exist(output)
            print("output: ", output)
            xy_name = "bat_XY"
            log_name = "log_file"
            event_matrix_name = "data"
            event_matrix_file, xy_file, log_path, logger = logger_utils.output_init(directory, event_matrix_name, xy_name, log_name, video_name)    
            ...
            while True: # Verifying 6 boxes, 6 holes and one carrier
                ...
                image_data = cv2.resize(frame, (env_input_size, env_input_size))
                ...
                if FLAGS.framework == 'tflite':
                    ...
                    if FLAGS.model == 'yolov3' and FLAGS.tiny == True:
                        boxes, pred_conf = filter_boxes(pred[1], pred[0], score_threshold=0.25,
                                                        input_shape=tf.constant([env_input_size, env_input_size]))
                    else:
            ...
            bat_input_size = FLAGS.size
            if FLAGS.framework == 'tflite':
                interpreter = tf.lite.Interpreter(model_path=FLAGS.weights_bats)
                interpreter.allocate_tensors()
                input_details = interpreter.get_input_details()
                output_details = interpreter.get_output_details()
                print(input_details)
                ...
                saved_model_loaded = tf.saved_model.load(FLAGS.weights_bats, tags=[tag_constants.SERVING])
                infer = saved_model_loaded.signatures['serving_default']
            ...
            while True:   
                ...
                if thresh_ind == 0 and sum(frame_q) < 5:
                    ...
                    frame_size = frame.shape[:2]
                    image_data = cv2.resize(frame, (bat_input_size, bat_input_size))
                    image_data = image_data / 255.
                    image_data = image_data[np.newaxis, ...].astype(np.float32)
                    start_time = time.time()
                    ...
                    if FLAGS.framework == 'tflite':
                        ...
                        if FLAGS.model == 'yolov3' and FLAGS.tiny == True:
                            boxes, pred_conf = filter_boxes(pred[1], pred[0], score_threshold=0.25,
                                                            input_shape=tf.constant([bat_input_size, bat_input_size]))
                        else:
                            boxes, pred_conf = filter_boxes(pred[0], pred[1], score_threshold=0.25,
                                                            input_shape=tf.constant([bat_input_size, bat_input_size]))
                    else:
            ...
            logic_utils.events_logger(directory, logger, event_matrix, oa_fps, log_path, video_name)
